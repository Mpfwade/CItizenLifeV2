...
concommand.Add("ix_selector_cwu", function(ply, cmd, args)
	...
	if not ply:NearEntity("ix_selector_cwu") then ply:ChatPrint("You need to be near the Worker quartermaster in order to use this!") return end
	if not (ply:Team() == FACTION_CITIZEN) then ply:ChatPrint("You need to become a Citizen to run this command.") return end
